...
const ANALYSIS_TYPE_ALIASES: Record<string, string[]> = {
  ...
  // Warfare sections - v9.0: Added edge function analysis_type aliases
  ...
  // v8.0 Phase 1 - Counter-Intelligence Sections (include edge function shorthand + full alias)
  dracoDeception: ['draco_deception', 'draco_deception_orchestrator'],
  sentientIntent: ['sentient_intent', 'sentient_intent_analysis'],
  insiderThreat: ['insider_threat_matrix'],
  bayesianIntention: ['bayesian_intention', 'bayesian_intention_prediction'],
  redTeamSimulator: ['red_team_adversary', 'red_team_adversary_simulation'],
  semaforForgery: ['semafor_forgery_detection', 'semafor_forgery'],
  epistemicVulnerability: ['epistemic_vulnerability', 'epistemic_vulnerability_scan'],
  cognitiveIW: ['cognitive_iw', 'cognitive_iw_detection'],
  
  // v8.0 Phase 2 - Psychological Warfare Sections
  psychoagentCascade: ['psychoagent_cascade', 'psychoagent_cascade_prediction'],
  affectiveManipulation: ['affective_manipulation', 'affective_manipulation_detection'],
  hyperpersonalization: ['hyperpersonalization', 'hyperpersonalization_mapping'],
  computationalPersuasion: ['computational_persuasion'],
  syntheticMemory: ['synthetic_memory', 'synthetic_memory_generation'],
  prememBelief: ['premem_belief', 'premem_belief_modification'],
  linguisticStress: ['linguistic_stress', 'linguistic_stress_detection'],
  memoryAnchor: ['memory_anchor', 'memory_anchor_generation'],
  emotionalContagion: ['emotional_contagion', 'emotional_contagion_modeling'],
  sacredValuePredictor: ['sacred_value', 'sacred_value_prediction'],
  ...
  ctdgLinkPredictor: ['ctdg_link', 'ctdg_link_prediction'],
  cascadeVirality: ['cascade_virality', 'cascade_virality_prediction'],
  networkResilience: ['network_resilience', 'network_resilience_analysis'],
  gazePattern: ['gaze_pattern', 'gaze_pattern_analysis'],
  microExpressionTimeline: ['micro_expression', 'micro_expression_timeline'],
  voiceStressCorrelator: ['voice_stress_correlation', 'voice_stress_correlator'],
  socialGraphPredictor: ['social_graph', 'social_graph_prediction'],
  behavioralFingerprint: ['behavioral_fingerprint'],
  
  // v8.0 Phase 4 - Doctrine & Prediction Sections
  influenceCampaignOptimizer: ['influence_campaign', 'influence_campaign_optimization'],
  counterNarrative: ['counter_narrative', 'counter_narrative_generation'],
  predictiveDoctrine: ['predictive_doctrine'],
  cognitiveDefenseSimulator: ['cognitive_defense', 'cognitive_defense_simulation'],
...
export function checkSectionHasData(
  ...
  const checks: Record<string, () => boolean> = {
    ...
    // v8.0 Phase 1 - Counter-Intelligence (check via hasAnalysis or dedicated data fields)
    dracoDeception: () => hasAnalysis(data, 'dracoDeception') || !!(data.dracoDeceptionData?.length),
    sentientIntent: () => hasAnalysis(data, 'sentientIntent') || !!(data.sentientIntentData?.length),
    insiderThreat: () => hasAnalysis(data, 'insiderThreat') || !!(data.insiderThreatData?.length),
    bayesianIntention: () => hasAnalysis(data, 'bayesianIntention') || !!(data.bayesianIntentionData?.length),
    redTeamSimulator: () => hasAnalysis(data, 'redTeamSimulator') || !!(data.redTeamAdversaryData?.length),
    semaforForgery: () => hasAnalysis(data, 'semaforForgery') || !!(data.semaforForgeryData?.length),
    epistemicVulnerability: () => hasAnalysis(data, 'epistemicVulnerability') || !!(data.epistemicVulnerabilityData?.length),
    cognitiveIW: () => hasAnalysis(data, 'cognitiveIW') || !!(data.cognitiveIwData?.length),
    
    // v8.0 Phase 2 - Psychological Warfare (check via hasAnalysis or dedicated data fields)
    psychoagentCascade: () => hasAnalysis(data, 'psychoagentCascade') || !!(data.psychoagentCascadeData?.length),
    affectiveManipulation: () => hasAnalysis(data, 'affectiveManipulation') || !!(data.affectiveManipulationData?.length),
    hyperpersonalization: () => hasAnalysis(data, 'hyperpersonalization') || !!(data.hyperpersonalizationData?.length),
    computationalPersuasion: () => hasAnalysis(data, 'computationalPersuasion') || !!(data.computationalPersuasionData?.length),
    syntheticMemory: () => hasAnalysis(data, 'syntheticMemory') || !!(data.syntheticMemoryData?.length),
    prememBelief: () => hasAnalysis(data, 'prememBelief') || !!(data.prememBeliefData?.length),
    linguisticStress: () => hasAnalysis(data, 'linguisticStress') || !!(data.linguisticStressData?.length),
    memoryAnchor: () => hasAnalysis(data, 'memoryAnchor') || !!(data.memoryAnchorData?.length),
    emotionalContagion: () => hasAnalysis(data, 'emotionalContagion') || !!(data.emotionalContagionData?.length),
    sacredValuePredictor: () => hasAnalysis(data, 'sacredValuePredictor') || !!(data.sacredValuePredictorData?.length),
    
    // v8.0 Phase 3 - Biometric & Network (check via hasAnalysis or dedicated data fields)
    pupillometry: () => hasAnalysis(data, 'pupillometry') || !!(data.pupillometryData?.length),
    thermalStress: () => hasAnalysis(data, 'thermalStress') || !!(data.thermalStressData?.length),
    attentionMultimodal: () => hasAnalysis(data, 'attentionMultimodal') || !!(data.attentionFuserData?.length),
    keystrokeDynamics: () => hasAnalysis(data, 'keystrokeDynamics') || !!(data.keystrokeDynamicsData?.length),
    sheafNeural: () => hasAnalysis(data, 'sheafNeural') || !!(data.sheafNeuralData?.length),
    ctdgLinkPredictor: () => hasAnalysis(data, 'ctdgLinkPredictor') || !!(data.ctdgLinkData?.length),
    cascadeVirality: () => hasAnalysis(data, 'cascadeVirality') || !!(data.cascadeViralityData?.length),
    networkResilience: () => hasAnalysis(data, 'networkResilience') || !!(data.networkResilienceData?.length),
    gazePattern: () => hasAnalysis(data, 'gazePattern') || !!(data.gazePatternData?.length),
    microExpressionTimeline: () => hasAnalysis(data, 'microExpressionTimeline') || !!(data.microExpressionData?.length),
    voiceStressCorrelator: () => hasAnalysis(data, 'voiceStressCorrelator') || !!(data.voiceStressCorrelatorData?.length),
    socialGraphPredictor: () => hasAnalysis(data, 'socialGraphPredictor') || !!(data.socialGraphPredictorData?.length),
    behavioralFingerprint: () => hasAnalysis(data, 'behavioralFingerprint') || !!(data.behavioralFingerprintData?.length),
    
    // v8.0 Phase 4 - Doctrine & Prediction (check via hasAnalysis or dedicated data fields)
    influenceCampaignOptimizer: () => hasAnalysis(data, 'influenceCampaignOptimizer') || !!(data.influenceCampaignData?.length),
    counterNarrative: () => hasAnalysis(data, 'counterNarrative') || !!(data.counterNarrativeData?.length),
    predictiveDoctrine: () => hasAnalysis(data, 'predictiveDoctrine') || !!(data.predictiveDoctrineData?.length),
    cognitiveDefenseSimulator: () => hasAnalysis(data, 'cognitiveDefenseSimulator') || !!(data.cognitiveDefenseData?.length),
