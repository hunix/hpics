  ...
  const { data: phaseSynergies, isLoading: synergiesLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).map((row: any) => ({
        ...
        successfulCascades: row.successful_cascades,
    ...
    mutationFn: async ({ phaseA, phaseB, success }: { phaseA: number; phaseB: number; success: boolean }) => {
      ...
      if (existing) {
        const newScore = success 
          ? Math.min(100, existing.synergy_score + 5)
          : Math.max(0, existing.synergy_score - 2);
        ...
        const { error } = await (supabase as any)
          ...
          .update({
            ...
            successful_cascades: success ? existing.successful_cascades + 1 : existing.successful_cascades,
