...
export function useAGISGlobalState() {
  ...
  const { data: globalState, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        id: data.id,
        phaseHealthScores: data.phase_health_scores || {},
        crossPhaseCorrelations: data.cross_phase_correlations || {},
        activeObjectives: data.active_objectives || [],
  ...
  const initializeGlobalState = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await (supabase as any)
        ...
        .insert({
          user_id: user.id,
          phase_health_scores: initialHealthScores,
  ...
  const synthesizeGlobalState = useMutation({
    mutationFn: async () => {
      ...
      const [
        ...
        (supabase as any).from('autonomous_campaigns').select('*', { count: 'exact', head: true }).eq('user_id', user.id).eq('is_active', true),
        (supabase as any).from('influence_cascades').select('*', { count: 'exact', head: true }).eq('user_id', user.id).eq('status', 'active'),
        (supabase as any).from('reality_frameworks').select('*', { count: 'exact', head: true }).eq('user_id', user.id).eq('is_active', true)
      ]);
