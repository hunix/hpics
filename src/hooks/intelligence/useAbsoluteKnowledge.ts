...
export function useAbsoluteKnowledge(profileId?: string) {
  ...
  const { data: knowledge, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).map((row: any): AbsoluteKnowledge => ({
        id: row.id,
        userId: row.user_id,
        ...
        knowledgePayload: row.knowledge_payload || {},
        derivationChain: row.derivation_chain || [],
    ...
    mutationFn: async (input: { knowledgeType: string; knowledgePayload?: Record<string, unknown> }) => {
      const { data, error } = await (supabase as any)
        ...
        .insert({
          ...
          knowledge_payload: input.knowledgePayload || {},
