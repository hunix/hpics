...
serve(async (req) => {
  ...
  try {
    ...
    const textCorpus = [
      ...
      ...(observations || []).map((o: { observation?: string }) => o.observation),
      ...(recordings || []).filter((r: { transcription?: string }) => r.transcription).map((r: { transcription?: string }) => r.transcription)
    ].filter(Boolean).join('\n\n');
    ...
    const contextData = {
      ...
      observationNotes: observations?.map((o: { observation?: string; category?: string }) => ({ note: o.observation, type: o.category }))
    };
