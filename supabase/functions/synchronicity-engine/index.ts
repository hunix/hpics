...
serve(async (req) => {
  ...
  if (__url.searchParams.get("healthCheck") === "1") {
    return new Response(JSON.stringify({ ok: true, function: "synchronicity-engine", timestamp: Date.now() }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
  ...
  try {
    ...
    const [interactionsRes, milestonesRes, anomaliesRes, predictionsRes] = await Promise.all([
      ...
      supabase.from('contact_life_milestones').select('*').eq('profile_id', profileId).gte('event_date', startDate.toISOString()),
