...
serve(async (req) => {
  ...
  try {
    ...
    const { profileId, signatureUrls, model: requestedModel } = await req.json();

    // Get user's preferred model or use default
    const analysisType = FUNCTION_TO_ANALYSIS_TYPE['extract-signature-biometrics'] || 'biometric_analysis';
...
JSON:
{
  ...
  "signature_profile": "unique signature description"
    ...
    const content: any[] = [
      ...
      ...signatureUrls.map((url: string) => ({ type: "image_url", image_url: { url } }))
    ];
    ...
    const aiResponse = await callAI({
      ...
      userId: user.id,
      functionName: 'extract-signature-biometrics',
      profileId,
      maxTokens: 1500,
    });
