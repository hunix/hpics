...
serve(async (req) => {
  ...
  try {
    ...
    const stats = {
      ...
      inboundCommunications: communications.filter(c => c.direction === 'inbound').length,
      outboundCommunications: communications.filter(c => c.direction === 'outbound').length,
      uniqueContactsReached: new Set(communications.map(c => c.profile_id)).size,
      eventsThisWeek: events.length,
      analysesGenerated: analyses.length,
      activeGoals: goals.length,
      goalsCompleted: goals.filter(g => g.last_completed_at && new Date(g.last_completed_at) >= weekStart).length
    };
