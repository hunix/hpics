-- Enhanced contacts search function with all filters and sorting
...
-- Get contact counts per first letter for alphabetical navigation
CREATE OR REPLACE FUNCTION public.get_contact_letter_counts(p_user_id uuid)
RETURNS TABLE (letter char(1), count bigint)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = 'public'
AS $$
  SELECT 
    UPPER(LEFT(first_name, 1))::char(1) as letter,
    COUNT(*) as count
  FROM profiles
...
-- Get available filter options (relationships, tags) efficiently
...
AS $$
  SELECT 
    ARRAY(SELECT DISTINCT relationship_type FROM profiles WHERE user_id = p_user_id AND relationship_type IS NOT NULL ORDER BY relationship_type),
