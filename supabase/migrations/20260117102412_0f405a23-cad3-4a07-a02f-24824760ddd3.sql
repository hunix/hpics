-- Drop ALL existing versions of search_contacts_v3 to eliminate ambiguity
DROP FUNCTION IF EXISTS public.search_contacts_v3(uuid, text, text, text, text, boolean, text, text, text, integer, integer);
...
DROP FUNCTION IF EXISTS public.search_contacts_v3(p_user_id uuid, p_search_query text, p_relationship_type text, p_relationship_subtype text, p_tag text, p_is_favorite boolean, p_first_letter text, p_sort_by text, p_sort_order text, p_limit integer, p_offset integer);
DROP FUNCTION IF EXISTS public.search_contacts_v3(p_user_id uuid, p_search_query text, p_relationship_type text, p_relationship_subtype text, p_tag text, p_is_favorite boolean, p_is_active boolean, p_first_letter text, p_sort_by text, p_sort_order text, p_limit integer, p_offset integer);

-- Create the single correct version with proper parameter order
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(p.engagement_score, 0)::integer as engagement_score,
